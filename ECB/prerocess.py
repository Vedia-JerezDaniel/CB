...
df = ecb[['date','Index','b','Rate','RateDecision']]
df = df.fillna(0)
...
df.to_excel("E:\\GitRepo\\CB speeches\\data\\ecb_pre.xlsx", engine='xlsxwriter')
...
rec_20081125 = pd.Series([True, False, False, 'Ben Bernanke', 0, -1, -1], index=['unscheduled', 'forecast', 'confcall', 'ChairPerson', 'Rate', 'RateDiff', 'RateDecision'], name=dt.datetime.strptime('2008-11-25', '%Y-%m-%d'))
    ...
    """Only apply the Taylor rule, and it is saved in a different Df
    """     
